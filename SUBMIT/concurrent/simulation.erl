...
spawnNodes(L, PriorityList ) ->
    ...
    case L of
        ...
        [{Pid, _, Name, Priority, Tolerance} | T] ->
            [spawn(agent,agent,[self(), -1, -1, Pid, Name, Priority, Tolerance, 
...
revolted(ListOfActors, NodesRevolted, Time, Leader ,Name, Agent, ListOfPriorities, StartTime, Stream) ->
    case lists:member(Name, NodesRevolted) of
        % if the agent that sent the message has already revolted 
        true -> 
            %agent can continue
            Agent ! {ok},
...
sort([Pivot|T]) ->
        sort([ X || X <- T, lessThan(X, Pivot)]) ++
            [Pivot] ++
                sort([ X || X <- T, greaterThan(X, Pivot)]);
sort([]) -> [].
...
run(Filename) ->
    {ok, Stream} = file:open("output" , write),
